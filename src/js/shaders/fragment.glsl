...
void main ()
{
  ...
  newFakeUV = fract(newFakeUV + vec2(time/6., time/4.));

  vec2 fakeUV = vec2(dot(vec3(1.), vNormal), dot(vec3(-1., 0., 1.), vNormal));
  fakeUV = fract(fakeUV + vec2(time/40., time/20.));

  // vec4 txt = texture2D(sky, vUv + 0.8 * cnoise(vec4(vUv * 20., time / 10., 0.), vec4(10.)));
  vec4 txt = texture2D(sky, newFakeUV + 0.2 * cnoise(vec4(fakeUV * 5., time / 100., 0.), vec4(10.)));
